...
class LabTestPackageDetailsPage extends StatefulWidget {
  ...
  @override
  State<LabTestPackageDetailsPage> createState() => _LabTestPackageDetailsPageState();
}
...
class _LabTestPackageDetailsPageState extends State<LabTestPackageDetailsPage> {
  ...
  Future<void> _fetchFullDetails() async {
    try {
      final updatedPackage = await _labTestService.getPackageDetails(widget.package.id);
      if (mounted) {
        setState(() {
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            _buildSectionHeader(Icons.info_outline, "Package Information", "Complete details about the package"),
            ...
            children: [
                ...
                children: [
                  ClipRRect(
                    ...
                    child: package.files.isNotEmpty
                        ? Image.network(
                            package.files.first, // Package API gives full URL usually or check
                            width: 80,
                            height: 80,
                            fit: BoxFit.cover,
                  ...
                  Positioned(
                    ...
                    child: Container(
                      ...
                      child: Text(package.status, style: GoogleFonts.inter(fontSize: 9, color: Colors.white, fontWeight: FontWeight.bold)),
                    ),
                  ),
                ],
              ),
              ...
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text(package.name, style: GoogleFonts.inter(fontSize: 16, fontWeight: FontWeight.bold, color: const Color(0xFF1E1B4B))),
                    const SizedBox(height: 8),
                    _chip("${package.products.length} Tests Included", Icons.biotech_outlined, Colors.indigo),
                  ],
                ),
              ),
            ],
  ...
  Widget _buildPriceArea(LabTestPackageItem package) {
    ...
    final int percent = package.price > 0 ? ((saving / package.price) * 100).toInt() : 0;
        ...
        children: [
          Column(
            ...
            children: [
              ...
              Text("₹${package.price.toInt()}", style: GoogleFonts.inter(fontSize: 13, color: Colors.grey[500], decoration: TextDecoration.lineThrough)),
            ],
          ),
          Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              ...
              Text("₹${package.discountPrice.toInt()}", style: GoogleFonts.inter(fontSize: 16, color: Colors.green[700], fontWeight: FontWeight.bold)),
            ],
          ),
          if (percent > 0)
            Container(
  ...
  Widget _buildInfoGrid(LabTestPackageItem package) {
    return Row(
      children: [
        _infoItem("TESTS COUNT", "${package.products.length}", Icons.biotech_outlined, Colors.blue),
        const SizedBox(width: 12),
        _infoItem("STATUS", package.status.toUpperCase(), Icons.check_circle_outline, Colors.green),
      ],
    );
  }
  ...
  Widget _buildPackageDetails(LabTestPackageItem package) {
    return Container(
      ...
      child: Column(
        children: [
           _detailRow("PACKAGE NAME", package.name, Icons.inventory_2_outlined, Colors.blue),
           const Divider(height: 1),
           _detailRow("STATUS", package.status, Icons.check_circle_outline, Colors.green),
        ],
      ),
    );
  }
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          Padding(
            padding: const EdgeInsets.all(16),
            child: _buildSectionHeader(Icons.assignment_outlined, "Included Tests", "Lab tests included in this package"),
          ),
          const Divider(height: 1),
          // Table Header
          Container(
            ...
            child: Row(
              children: [
                Expanded(flex: 3, child: Text("TEST NAME", style: GoogleFonts.inter(fontSize: 10, fontWeight: FontWeight.bold, color: Colors.grey[400], letterSpacing: 0.5))),
                Expanded(flex: 2, child: Text("SAMPLE TYPE", style: GoogleFonts.inter(fontSize: 10, fontWeight: FontWeight.bold, color: Colors.grey[400], letterSpacing: 0.5))),
                Expanded(flex: 2, child: Text("REPORT DURATION", style: GoogleFonts.inter(fontSize: 10, fontWeight: FontWeight.bold, color: Colors.grey[400], letterSpacing: 0.5))),
                Expanded(flex: 1, child: Text("FASTING", style: GoogleFonts.inter(fontSize: 10, fontWeight: FontWeight.bold, color: Colors.grey[400], letterSpacing: 0.5))),
              ],
            ),
          ),
          ...tests.map((t) => Container(
                ...
                child: Row(
                  children: [
                    Expanded(flex: 3, child: Text(t.name, style: GoogleFonts.inter(fontSize: 11, fontWeight: FontWeight.bold, color: const Color(0xFF1E1B4B)))),
                    Expanded(flex: 2, child: Text(t.sampleType ?? "N/A", style: GoogleFonts.inter(fontSize: 11, color: const Color(0xFF475569)))),
                    Expanded(flex: 2, child: Text(t.reportsDuration ?? "N/A", style: GoogleFonts.inter(fontSize: 11, color: const Color(0xFF475569)))),
                    Expanded(flex: 1, child: _smallBadge(t.isFasting?.toLowerCase() == 'yes')),
                  ],
                ),
              )),
          if (tests.isEmpty)
             const Padding(padding: EdgeInsets.all(24), child: Center(child: Text("No tests included in this package"))),
        ],
      ),
    );
  }
  ...
  Widget _smallBadge(bool isYes) {
    return Container(
      ...
      decoration: BoxDecoration(color: (isYes ? Colors.yellow[100] : Colors.green[50]), borderRadius: BorderRadius.circular(4)),
      child: Text(isYes ? "Yes" : "No", style: GoogleFonts.inter(fontSize: 9, fontWeight: FontWeight.bold, color: (isYes ? Colors.orange[800] : Colors.green[700]))),
    );
  }
