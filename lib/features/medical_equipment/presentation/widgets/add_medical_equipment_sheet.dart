...
class _AddMedicalEquipmentSheetState extends State<AddMedicalEquipmentSheet> {
  ...
  final MedicalEquipmentService _service = MedicalEquipmentInjection.provideMedicalEquipmentService();
  ...
  void initState() {
    ...
    if (isEditMode) {
      ...
      _returnChargeController.text = item.returnCharge?.toInt().toString() ?? "0";
      _serviceChargeController.text = item.serviceCharges?.toInt().toString() ?? "0";
  ...
  Future<void> _submit() async {
    ...
    if (_selectedTabletId == null) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Please select an equipment'), backgroundColor: Colors.red));
      return;
    }
              ...
              children: [
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      Text(
                        isEditMode ? 'Edit Medical Equipment' : 'Add New Medical Equipment',
            ...
            child: SingleChildScrollView(
              ...
              child: Form(
                ...
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text("Medical Equipment Information", style: GoogleFonts.inter(fontSize: 15, fontWeight: FontWeight.bold, color: const Color(0xFF1E1B4B))),
                    Text("Please provide accurate information for the medical equipment", style: GoogleFonts.inter(fontSize: 12, color: Colors.grey[500])),
                    const SizedBox(height: 20),

                    // Search Name
                    _buildLabel("Medical Equipment Name", isRequired: !isEditMode, icon: isEditMode ? Icons.medical_information_outlined : Icons.search),
                    ...
                    Row(
                      children: [
                        Expanded(child: _buildInputField("Fixed Deposit (₹)", _depositController, Icons.savings_outlined)),
                        const SizedBox(width: 12),
                        Expanded(child: _buildInputField("Return Charge (₹)", _returnChargeController, Icons.replay_outlined)),
                      ],
                    ),
                    const SizedBox(height: 16),
                    Row(
                      children: [
                        Expanded(child: _buildInputField("Service Charges (₹)", _serviceChargeController, Icons.design_services_outlined)),
                        const SizedBox(width: 12),
                         Expanded(
                           child: Column(
                             crossAxisAlignment: CrossAxisAlignment.start,
              ...
              children: [
                ...
                SizedBox(
                  ...
                  child: ElevatedButton.icon(
                    ...
                    label: Text(isEditMode ? "Update Equipment" : "Add Equipment", style: GoogleFonts.inter(fontSize: 14, fontWeight: FontWeight.bold)),
