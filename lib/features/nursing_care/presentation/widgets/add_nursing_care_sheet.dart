...
import 'package:MediCompare/features/nursing_care/data/data_sources/nursing_care_service.dart';
...
class AddNursingCareSheet extends StatefulWidget {
  final NursingCareItem? editItem;
  final VoidCallback onSuccess;

  const AddNursingCareSheet({super.key, this.editItem, required this.onSuccess});

  @override
  State<AddNursingCareSheet> createState() => _AddNursingCareSheetState();
}
...
class _AddNursingCareSheetState extends State<AddNursingCareSheet> {
  ...
  List<NursingCareDropdownItem> _searchResults = [];
  Timer? _debounce;
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      if (isEditMode) {
        await _service.updateNursingCare(widget.editItem!.id, payload);
      } else {
        await _service.createNursingCare(payload);
      }
      widget.onSuccess();
