...
class _HomeCareListPageState extends State<HomeCareListPage> {
  ...
  void _loadInitial() {
    context.read<HomeCareBloc>().add(const LoadHomeCareListEvent(page: 1, isRefresh: true));
  }

  void _onScroll() {
    if (_scrollController.position.pixels >= _scrollController.position.maxScrollExtent * 0.8) {
      final state = context.read<HomeCareBloc>().state;
      if (state.status != HomeCareStatus.loading && state.pagination != null && state.pagination!.page < state.pagination!.totalPages) {
  ...
  Future<void> _deleteItem(String id) async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: const Text("Are you sure you want to delete this healthcare service?"),
        actions: [
        ...
        builder: (context, state) {
          return Column(
            children: [
                     ...
                     Container(
                       ...
                       child: DropdownButtonHideUnderline(
                         child: DropdownButton<String>(
                           ...
                           isExpanded: true,
                           items: [
                             DropdownMenuItem(value: '', child: Text("All Categories", style: GoogleFonts.inter(fontSize: 14))),
                             ...state.categories.map((c) => DropdownMenuItem(value: c.id, child: Text(c.name, style: GoogleFonts.inter(fontSize: 14)))),
                           ],
                           onChanged: (val) => context.read<HomeCareBloc>().add(SelectHomeCareCategoryEvent(val ?? '')),
                         ),
                       ),
                     ),
                   ],
               ...
               // List
               Expanded(
                 child: state.status == HomeCareStatus.loading && state.items.isEmpty
                     ? const Center(child: CircularProgressIndicator())
                     : state.items.isEmpty
                         ? Center(child: Column(
                            mainAxisAlignment: MainAxisAlignment.center,
                            children: [
                              Icon(Icons.home_repair_service_outlined, size: 64, color: Colors.grey[300]),
                              const SizedBox(height: 16),
                              Text("No Home Care Services Found", style: GoogleFonts.inter(fontSize: 16, fontWeight: FontWeight.bold, color: Colors.grey[600])),
                            ],
                          ))
                         : RefreshIndicator(
                             ...
                             child: ListView.builder(
                               ...
                               itemCount: state.items.length + (state.status == HomeCareStatus.loading ? 1 : 0),
                               itemBuilder: (context, index) {
                                 if (index < state.items.length) {
                                   final item = state.items[index];
                                   return HomeCareCard(
                                     item: item,
                                     onView: () => context.push('/homecare-details', extra: item),
