...
class AddMedicineSheet extends StatefulWidget {
  ...
  const AddMedicineSheet({super.key, required this.onSuccess, this.editMedicine});
...
class _AddMedicineSheetState extends State<AddMedicineSheet> {
  ...
  final MedicineService _medicineService = MedicineInjection.provideMedicineService();
  ...
  Future<void> _loadEditData() async {
    ...
    try {
      final data = await _medicineService.getVendorMedicineDetails(widget.editMedicine!.id);
      ...
      setState(() {
        ...
        _discountController.text = (data['discountprice'] ?? data['discount'] ?? 0).toString();
        _quantityController.text = (data['quantity'] ?? 1).toString();
        _pricePerUnitController.text = (data['pricePerUnit'] ?? '').toString();
        _percentagePerUnitController.text = (data['percentagePerUnit'] ?? '').toString();
                ...
                children: [
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        Text(isEditMode ? "Edit Medicine" : "Add New Medicine", style: GoogleFonts.poppins(fontSize: 18, fontWeight: FontWeight.bold, color: const Color(0xFF1E1B4B))),
                        Text(isEditMode ? "Update the medicine information" : "Fill in the basic Medicine information", style: GoogleFonts.poppins(fontSize: 12, color: Colors.grey[600])),
                      ],
                    ),
                  ),
                  IconButton(
              ...
              child: SingleChildScrollView(
                ...
                child: Form(
                  ...
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text("Medicine Information", style: GoogleFonts.poppins(fontSize: 16, fontWeight: FontWeight.bold, color: const Color(0xFF1E1B4B))),
                      const SizedBox(height: 4),
                      Text(isEditMode ? "Update the medicine details below" : "Please provide accurate information for the medicine", style: GoogleFonts.poppins(fontSize: 12, color: Colors.grey[600])),
                      const SizedBox(height: 24),
                      
                      // Medicine Name Input
                      _buildLabel("Medicine Name", isRequired: true, icon: Icons.link),
                      ...
                      if (isEditMode && _selectedTablet != null) ...[
                        Container(
                          ...
                          child: Row(
                            ...
                            children: [
                              Flexible(
                                child: Text(_selectedTablet!.name, style: GoogleFonts.poppins(fontSize: 14, color: const Color(0xFF4B5563)), overflow: TextOverflow.ellipsis),
                              ),
                      ...
                      DropdownButtonFormField<int>(
                        value: _selectedReturnPolicy != null ? int.tryParse(_selectedReturnPolicy!) : null,
                        decoration: _inputDecoration(hint: "Select Return Policy"),
                        items: _returnPolicies.map((p) => DropdownMenuItem<int>(
                          value: p['value'] as int,
                          child: Text(p['label'] as String, style: GoogleFonts.poppins(fontSize: 14)),
                        )).toList(),
                        onChanged: (val) => setState(() => _selectedReturnPolicy = val?.toString()),
                        validator: (val) => val == null ? "Required" : null,
                      ),
                      
                      if (_selectedTablet != null) ...[
                        const SizedBox(height: 32),
                        Text("Medicine Details", style: GoogleFonts.poppins(fontSize: 16, fontWeight: FontWeight.bold, color: const Color(0xFF1E1B4B))),
                        ...
                        Container(
                          ...
                          child: Column(
                            ...
                            children: [
                              Row(
                                children: [
                                  Expanded(child: _buildStaticField("Medicine Name", _selectedTablet!.name, Icons.link)),
                                  const SizedBox(width: 12),
                                  Expanded(child: _buildTextField("MRP Price (₹)", _mrpPriceController, hint: "0.00", icon: Icons.currency_rupee)),
                                ],
                              ),
                              const SizedBox(height: 16),
                              Row(
                              ...
                              Row(
                                children: [
                                  Expanded(child: _buildTextField("Price/Units", _pricePerUnitController, hint: "e.g., ₹10/tab", icon: Icons.currency_rupee, isRequired: false)),
                                  const SizedBox(width: 12),
                                  Expanded(child: _buildTextField("Percentage/Unit", _percentagePerUnitController, hint: "e.g., 10%", icon: Icons.percent, isRequired: false)),
                                ],
                              ),
                              const SizedBox(height: 24),
                              Row(
                                    ...
                                    child: Column(
                                      ...
                                      children: [
                                        ...
                                        _buildToggleBtn(
                                          ...
                                          _isInStock ? Icons.check_circle_outline : Icons.cancel_outlined, 
                                          _isInStock ? Colors.green : Colors.red,
                                          () => setState(() => _isInStock = !_isInStock),
                                        ),
                                      ],
                                    ),
                                  ),
                                    ...
                                    child: Column(
                                      ...
                                      children: [
                                        ...
                                        _buildToggleBtn(
                                          ...
                                          _isActive ? const Color(0xFF506CCF) : Colors.grey,
                    ...
                    child: Row(
                      children: [
                        ...
                        ElevatedButton.icon(
                          ...
                          icon: _isLoading ? const SizedBox(width: 16, height: 16, child: CircularProgressIndicator(color: Colors.white, strokeWidth: 2)) : const Icon(Icons.link, size: 16),
                          label: Text(_isLoading ? "Saving..." : (isEditMode ? "Update Medicine" : "Add Medicine"), style: GoogleFonts.poppins(fontWeight: FontWeight.w600)),
                        ),
                      ],
                    ),
                  )
    ...
    return Column(
      ...
      children: [
        ...
        Container(
          ...
          child: Text(value, style: GoogleFonts.poppins(fontSize: 11, color: const Color(0xFF1E1B4B)), overflow: TextOverflow.ellipsis, maxLines: 1),
        ),
      ],
    );
  }
